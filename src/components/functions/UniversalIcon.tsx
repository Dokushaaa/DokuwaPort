...
const UniversalIcon: React.FC<UniversalIconProps> = ({
  ...
  const componentName = formatIconName(
    packPrefix.charAt(0).toUpperCase() +
      packPrefix.slice(1),
    iconName
  );
  ...
  if (!IconComponent) {
    console.warn(
      `Icon "${componentName}" not found in "${packPrefix}".`
    );
    return null;
  }
...
export const UniversalIconExample2 = () => {
